// IMPROVED: Centralized CSV utilities for better maintainability
        ...
        const DOMCache = {
            ...
            init() {
                const elementIds = [
                    ...
                    'homeCollapsed', 'workCollapsed', 'teamCollapsed', 'notesCollapsed',
                    'editModal', 'categoryModal', 'teamModal', 'deadlineModal', 'deleteModal',
                    ...
                    'currentDate', 'undoBtn',
                    'notesEditor', 'recentlyDeletedList'
                ];
        ...
        const app = {
            ...
            handleSectionDragOver(event) {
                event.preventDefault();
                const sectionEl = event.target.closest('.section');
                if (sectionEl && this.draggedSection && sectionEl.dataset.section !== this.draggedSection) {
                    const dayView = DOMCache.get('dayView');
                    const draggedEl = dayView.querySelector(`[data-section="${this.draggedSection}"]`);
                    
                    if (draggedEl && sectionEl) {
                        const rect = sectionEl.getBoundingClientRect();
                        const midpoint = rect.top + rect.height / 2;
                        
                        if (event.clientY < midpoint) {
                            dayView.insertBefore(draggedEl, sectionEl);
                        } else {
                            dayView.insertBefore(draggedEl, sectionEl.nextSibling);
                        }
                    }
                }
            },
            ...
            handleSectionDragEnd(event) {
                ...
                const dayView = DOMCache.get('dayView');
                const sections = Array.from(dayView.querySelectorAll('.section'));
            ...
            handleDragOverTask(event) {
                if (this.draggedCategory) {
                    event.preventDefault();
                    event.dataTransfer.dropEffect = 'copy';
                    const taskEl = event.target.closest('.task');
                    if (taskEl && taskEl.dataset.taskSection === 'work') {
                        taskEl.classList.add('drag-over-category');
                    }
                } else if (this.draggedTask) {
                    event.preventDefault();
                    const taskEl = event.target.closest('.task');
                    if (taskEl && taskEl !== this.draggedTask) {
                        const container = taskEl.parentElement;
            ...
            handleDragLeaveTask(event) {
                const taskEl = event.target.closest('.task');
                if (taskEl) {
                    taskEl.classList.remove('drag-over-category');
                }
            },

            handleDropOnTask(event) {
                event.preventDefault();
                const taskEl = event.target.closest('.task');
                taskEl.classList.remove('drag-over-category');

                if (this.draggedCategory && taskEl.dataset.taskSection === 'work') {
                    ...
                    if (task) {
                        ...
                        if (!task.workCategories.includes(this.draggedCategory)) {
                            task.workCategories.push(this.draggedCategory);
                            this.updateUI();
                            
                            taskEl.style.transition = `all ${this.CONFIG.ANIMATION.DRAG_FEEDBACK / 1000}s`;
                            taskEl.style.transform = 'scale(1.05)';
                            setTimeout(() => {
                                taskEl.style.transform = 'scale(1)';
                            }, this.CONFIG.ANIMATION.DRAG_FEEDBACK);
                        }
                    }
                }
            },
            ...
            handleSectionDragOver(event) {
                ...
                if (this.draggedSection && section !== this.draggedSection) {
                    ...
                    const allSections = Array.from(container.children).filter(el => 
                        el.classList.contains('section') || el.id === 'notesSection'
                    );
            ...
            handleSectionDragEnd(event) {
                ...
                this.sectionOrder = sections.map(section => {
                    if (section.id === 'notesSection') return 'notes';
